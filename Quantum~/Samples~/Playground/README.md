...
2. **Config (optional):** without one, characters use the built-in defaults. To tune them:
   *Create → Quantum → Physics Player Controller → Character Config*, kept under `Assets/` (Quantum only
   indexes `AssetSearchPaths`, which is `Assets` by default). Characters get a frictionless physics
   material automatically unless you set **Body → Material**.
...
6. **RuntimeConfig** (e.g. on `QuantumRunnerLocalDebug`): set `Playground Character View`, and
   optionally `Playground Character Config` and `Playground Spawn Point` (feet position; default origin).
